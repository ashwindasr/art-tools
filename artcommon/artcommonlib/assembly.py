...
def assembly_permits(releases_config: Model, assembly: typing.Optional[str]) -> ListModel:
    ...
    if assembly_type(releases_config, assembly) == AssemblyTypes.STANDARD:
        if any(permit.code not in [AssemblyIssueCode.EMBARGOED_CONTENT.name, AssemblyIssueCode.CONFLICTING_INHERITED_DEPENDENCY.name] for permit in defined_permits):
